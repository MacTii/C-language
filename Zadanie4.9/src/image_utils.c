...
{
    if (img == NULL || img->ptr == NULL || img->width < 1 || img->height < 1 || src == NULL || src->ptr == NULL || src->height < 1 || src->width < 1 ||
        destx < 0 || desty < 0 || destx >= img->width || desty >= img->height || src->width > img->width || src->height > img->height ||
        desty + src->height > img->height || destx + src->width > img->width)
        return INCORRECT_INPUT;

    for (int i = desty, i_src = 0; i < desty + src->height; i++, i_src++) // (16,15) desty = 15 src->height=1 | i=15 ; i < 15+1 ; i++
    {
        for (int j = destx, j_src = 0; j < destx + src->width; j++, j_src++) // destx = 16 src->width=1 | j=16 ; j<16+1 ; j++
            *(*(img->ptr + i) + j) = *(*(src->ptr + i_src) + j_src);
    }
    return SUCCESS;
}